...
def pipe_pick(NumeroQuestion):
    PipeId = math.floor(NumeroQuestion/2)
    if PipeId < 0:
        PipeId=0
    if PipeId > 5:
        PipeId=5
    ...
    if TimeLeft > 0:
        ...
        if Start:
            ...
            if TimeLeft < 45 and NumeroQuestion == 1:
                ...
                if ((NumeroQuestion+QuestionOffset)%14+4) in QuestionAsked:
                    QuestionOffset += 1
            ...
            if TimeLeft < 25 and NumeroQuestion == 3:
                ...
                if ((NumeroQuestion+QuestionOffset)%14+8) in QuestionAsked:
                    QuestionOffset += 1
            ...
            if trigger and ((52 < TimeLeft <= 53) or 42 < TimeLeft <= 43 or 32 < TimeLeft <= 33 or 22 < TimeLeft <= 23 or 12 < TimeLeft <= 13 or  3 < TimeLeft <= 5):
                subprocess.Popen(["curl", "--silent", "-o /dev/null", "http://wled.local/win&PL=3"], start_new_session=True)
                trigger = False
